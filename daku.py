...
TELEGRAM_BOT_TOKEN = '7908873813:AAG58xV2v3NgeWjeuEiwyRYbpK82sxGYM60'
ADMIN_USER_ID = 1813707353
...
async def daku(update: Update, context: CallbackContext):
    ...
    if len(args) != 2:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /daku <add|rem> <user_id>*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Another attack is already in progress. Please wait 180 SECONDS.*", parse_mode='Markdown')
        return
